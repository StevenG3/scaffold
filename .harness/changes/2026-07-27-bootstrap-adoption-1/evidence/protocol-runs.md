...
```sh
git show --stat HEAD
git diff --name-status 1ee71612..HEAD
git rev-list --count 1ee71612..HEAD        # 在交付 HEAD 上应为一
...
| `下一轮外审是第 12 轮`（已过期的前瞻句） | 0 | 外审第 12 轮记录落盘后该句即过期，R29 全部改真为「下一轮第 13 轮」。**扫描目标由错误标签换成过期前瞻句**——`第 13 轮` 已成为正确标签，继续扫它只会扫到真话 |
...
| prose_carriers / 单行 docstring | 代码实现、SSOT、指针或节级历史域 |
...
| **外审轮次口径** | `git status --porcelain docs/reviews \| grep -c '^??.*pr-7'`；`git ls-files docs/reviews \| wc -l`；`git ls-files docs/reviews \| grep -c pr-7` | 依次为 **13**、**14**、**0**。外审文件现覆盖 R1–R13，故**下一轮外审为第 14 轮**。第一条依赖审阅方的落盘工作区，fresh clone 得零属预期；此前「`docs/reviews/` 有意不纳入版本控制」一句为假声称，缘由见 `summary.md` 的 R24 整改记录。**本行在 R28 交付时曾为红，R30 交付前又被外审第 13 轮记录推进了一次**——外审记录落盘会改变第一条的计数，而该行的期望不会自己跟上；凡「审阅方落盘」这类外部事件改变的计数，必须在下一轮开头复跑本行。 |
...
| **非 ASCII 片段边界与行终止符（R31/R32）** | `python3 .harness/changes/2026-07-27-bootstrap-adoption-1/evidence/check_current_numbers.py --self-test \| grep -E '非 ASCII\|non-ASCII\|semicolon\|fails closed'` | 三条非 ASCII 边界行与「分号后字符串不是 docstring」行全 `ok`，两条编码不可满足行 `read fails closed`。`col_offset` 是 **UTF-8 字节偏移**，故片段一律取自 `ast.get_source_segment`，不自行按列切；R30 的双向用例边界全在 ASCII 上，所以字符下标切片全绿而缺陷仍在——**边界用例的边界必须落在会出错的字符上。** R32 另补三行：裸 CR 与 CRLF 的 docstring 须与 LF **等价**（段内按 `\r\n` / `\r` / `\n` 三者显式拆分），而 U+2028 **不**被当作终止符——`str.splitlines()` 会在那里多切一刀，正是两轮前把合法注释洗成代码的同一个坑。 |
