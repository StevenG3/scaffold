...
- [x] F3(b)：撤回长度 3–6 的全域措辞，精确限定为「200,000 次抽取、137,527 个唯一输入、仅对被抽到的输入成立」；manifest / 生成表 / summary 统一。（本条及以上各轮条目中的「20 万」为**当轮历史措辞**，非当前口径；当前口径一律为「200,000 次抽取 / 137,527 个唯一输入」。）
...
- [x] Minor 1 / Minor 2：抽样层措辞更正并入审计表；默认语义在**脚本 docstring、run-manifest 模式表、summary 三处**同步为「四类自检 + 穷举层」（R8b 复核后确认为 3 处，另在生成表表头新增认证自检行）。
- [x] 自跑审阅协议第 5 步绿灯电池与第 6 步变异探针；7 条门禁、零回灌、推送、远端 CI。
- [x] 会话 A（Standards）与会话 B（Spec）盲审：双双 request-changes，反例重合。

## R8b 整改（三会话协议自验 d3290cdd）

- [x] R1：`CHECK_CLASSES` 成为唯一来源，`CERT_LAYERS` 由其派生并被实际消费；新增「未注册自检失败」兜底层，前缀未注册即 fail closed；两条回归（认证自检失败 / 未注册前缀）实测均 exit 1 + 拒绝写入 + 不认证标记。
- [x] R2：刷新脚本 SHA-256；门禁载体行按 §2(b) 重录为当前最后一个非空行；`_digest` 打桩探针 digest 重跑重录。
- [x] R3：提交前最后一步在最终内容上重跑收集器并迭代到不动点（267 行，逐文件分布）；新增审计第 18–24 行；删除概括兜底句；表基改为「最终提交态命中集 + 人工分类」。
- [x] R4：digest 收窄落到 summary 两处；tasks「三处同步」订正。
- [x] R5：生成表层状态表与表头、run-manifest 结果表均含认证自检行（**当轮历史值 12/12**；当前值见 run-manifest 当前结果表）。
- [x] R6：USAGE 重写；fixture 显式标注；docstring 计数订正为十二项；历史「20 万」标注；层数表述统一。
- [x] 重跑协议第 5 步电池、第 6 步探针与两个会话的反例；7 条门禁、零回灌、推送、远端 CI。
- [x] 会话 C 完整机械协议：确认 A/B 全部发现已闭合（六维 22/24），另抓出 4 项。

## R8c 整改（会话 C 机械协议校验 e7f3d722）

- [x] NC-1：订正 `summary.md` 与 `tasks.md` 中的收集器计数；在所有内容编辑之后重跑收集器迭代至不动点，三处记录值与最终实测一致。
- [x] NC-2：`CHECK_CLASSES` 每项携带产出者；执行数为 0 的注册类 fail closed；回归含逐前缀断言与真实 orphan 类注入（default 与 emit PATH 均 exit 1，正式文件未改写）。
- [x] NC-3：表头 attestation 由注册表派生；回归断言每个注册类同时出现在层状态表与 attestation。
- [x] NC-4：断言未认证输出的第一行即横幅首行（已认证输出反之）；删除横幅实测该项 FAIL 且 exit 1。
- [x] 重跑协议第 5 步电池与第 6 步探针；7 条门禁、零回灌、推送、远端 CI。
- [x] 外部审阅方第 9 轮独立复审（72bd0a4）：3 项 Important + 1 项 Minor。

## R9 整改（外部复审 72bd0a4）

- [x] F1：`certification_state()` 只返回 layer 事实；各出口从自身渲染的行现场派生裁决；新增与派生无共享的产物级写入守卫；残余风险（伪造事实本身）已披露。
- [x] F1 回归：审阅方原始变异形状现已无对象可污染，实测 rc=1、横幅首行、无 CERTIFIED 标记；伪造渲染实测被守卫拒绝。
- [x] F2：统一产出者命名约定 + 导入期发现断言；未注册产出者实测导入失败 exit 1、不写文件；文档声明收窄为四条实际保证。
- [x] F3(a)：删除全部「唯一判定模式」句，改为模式矩阵（唯一权威）。
- [x] F3(b)：当前结果统一到认证自检 23 项；历史值加「当轮历史值」标注。
- [x] F3(c)：抽样句全域统一为「每个命中输入做独立 oracle 分支 + 载体核对；不能证明的是未抽中输入」。
- [x] F3(d)：内容改完后重跑收集器至不动点；补审计行 19b / 20b / 21b。
- [x] 自跑审阅方三个探针 + 协议第 5 步电池；7 条门禁、零回灌、推送、远端 CI。
- [ ] 新的内部验证者复核；通过后交外部审阅方第 10 轮。
