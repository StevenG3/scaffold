...
- [x] 扫描方法学更正：定向用例逐条携带期望分支，从流程 §2.3 对抗域逐项取材，并加测集合外边界邻居；在运行清单中写明当时的随机模糊测试无法验证判据本身（**当轮历史措辞**；当前抽样层已对每个命中输入核对分支与载体）。
...
- [x] F-R10-3：脚本 SHA 历史值加「当轮历史值」标注，当前值统一在 run-manifest 参数表。
...
- [x] I2：抽样句改为按时点限定（旧扫描器 / 当前抽样层），全仓库记录复扫。
- [x] I3：新建 `evidence/protocol-runs.md`，会话 C 运行入库为历史条目；本轮机械步骤由实现方亲自执行并记为当前条目。
...
- [x] 2：`_digest` 打桩块与红基线转录**重新执行并逐字粘贴**；当前语态 69 → 74；历史裸值加当轮历史标注（含 R8 行的认证自检 9 项）。
...
- [x] 1：`protocol-runs.md` 当前条目在**最终内容态**重跑并粘贴本提交实测值（脚本 SHA / 摘要 / 定向数 / 前驱 SHA / 增量统计）。
...
- [x] 1：**去镜像推到脚本可导出的一类**——当前语态记录不再手抄定向数 / 自检项数 / 穷举与抽样总数 / 分支直方图 / 探针 digest；改为命令 + 机器生成的 `boundary-cases.md`；仅保留脚本 SHA 与结果摘要两个产物不变量。
...
- [x] C1：`run-manifest.md:49` 的过期当前态值删除；同处 69 作为 R7 历史值保留并绑定当轮 HEAD。
