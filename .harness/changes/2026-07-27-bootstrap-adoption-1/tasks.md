...
- [x] 扫描方法学更正：定向用例逐条携带期望分支，从流程 §2.3 对抗域逐项取材，并加测集合外边界邻居；在运行清单中写明当时的随机模糊测试无法验证判据本身（该轮历史措辞，绑定当轮 HEAD `50a6060`；现行抽样层已对每个命中输入核对分支与载体）。
...
- [x] 新增 `--baseline`：红基线实测该轮 69 个用例中 30 个见红（历史值，绑定当轮 HEAD `50a6060`），同批用例在其后判据下全绿。
...
- [x] F3(b)：撤回长度 3–6 的全域措辞，精确限定为「200,000 次抽取、137,527 个唯一输入、仅对被抽到的输入成立」；manifest / 生成表 / summary 统一。（本条及以上各轮条目中的「20 万」为该轮历史措辞，绑定当轮 HEAD `002b5d2`；现行口径的抽取数与唯一输入数读 机器生成的 `boundary-cases.md`。）
...
- [x] F-R10-1：路径预检 + `OSError` 兜底，非法路径一律 exit 2；argv 自检增至 27 项；订正被证伪的注释；模式矩阵每一格逐条实测并记录。
- [x] F-R10-2(a)：守卫新增证言完整性检查（CERTIFIED 在场时 `X/Y` 必须相等），验证者探针实测被拒。
- [x] F-R10-2(b)：残余披露改写为按层精确表——定向 / 四类自检 / 未注册标签层可捕获；穷举层与抽样层伪造未覆盖，如实声明。
- [x] F-R10-3：脚本 SHA 历史值加轮次标注，实测值统一读 机器生成的 `boundary-cases.md` 与 run-manifest 不变量表。
...
- [x] F-R11-1：三项路径自检改为在自建私有临时目录内构造与清理，不读共享 `$TMPDIR` 名、cwd 或绝对根路径；`target_path_problem()` 未动；确定性陈述按实情重新限定；验证者 `mkdir` 探针实测不再影响任何检查。
- [x] F-R11-1 顺带：新辅助函数被导入期发现断言拦下（命名符合产出者约定却非产出者），改名 `argv_path_checks`。
- [x] F-R11-2：守卫扫描层详情数字，非零缺陷计数在 CERTIFIED 下即拒绝；两个外科式伪造实测被拒并固化为自检回归。
...
- [x] A-I-A：抽出唯一的 `render_certification_block()`，默认模式与 emit 出口渲染同一输出并运行同一守卫；A 的两个探针在默认模式下实测 rc=1 且打印 GUARD REFUSED。
...
- [x] I1：认证视图由哨兵行界定，守卫只解析该视图；三种载荷字面量固化为定向用例（69 → 72），四项回归全部实测通过。
...
- [x] Minors：审计第 12 行导入清单补 `re`/`shutil`；新增审计第 32 行（哨兵不可伪造，含残余）；收集器词表扩充 `不可能`/`must not`/`never` 并写入「词表随声明词汇增长」的规则；`protocol-runs.md` 当前条目约定成文。
...
- [x] 1：`protocol-runs.md` 现刻条目在**最终内容态**重跑并粘贴本提交实测值（脚本 SHA / 摘要 / 前驱 SHA）。
...
- [x] Minor：起始哨兵字面量作为定向用例提交（74 → 75），实测 PASS。
...
- [x] 2：`argv_path_checks()` 的文件系统构造包 `try/except OSError`，失败降级为 argv 层内具名失败检查；审计第 28 行改判为「已限定」并枚举已闭合项与 `umask` 这一已承认项；`umask 177` / `022` 双向实测。
