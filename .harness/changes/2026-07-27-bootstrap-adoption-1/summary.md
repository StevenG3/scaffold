...
| `evidence/boundary-cases.md` | 69 个定向用例逐条表，**由 `--emit-markdown` 生成**，含**期望载体与实际载体两列**，输入与载体均以完整 `repr` 呈现、不截断。不得手工编辑。 |
...
exit=0
stdout: result digest (sha256 of sorted result lines, inputs and actual carriers bound in): 179b7a01c134cf70f68606c939e155390f0c21113811172a2f8d43e88fca63c6
        （同流附记：directed cases: 69, failures: 0 / self-test failures: 0 / undefined/exception cases: 0）
stderr: <empty>
```
...
定向用例 69 个、失败 0（**分支与载体值双重比对**）；比较器自检 5 项、argv 自检 9 项均通过；随机迭代 20 万、逃出划分 0；五个分支全部被覆盖，无死分支。
结果摘要**绑定输入字节与实际载体值**。
