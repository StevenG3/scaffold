...
以上九个提交均不含本 Change Record 的任何审计元数据；资产 diff 即证据，未另做副本。本 Change Record 落在其后的审计提交中。
...
扫描证据以文件形式随本 Change Record 交付，位于 `evidence/`（本目录属审计元数据）：

| 文件 | 内容 |
| --- | --- |
| `evidence/carrier_sweep.py` | 载体规则的机械实现 + 扫描器。仅标准库、seed 写死在常量中、无时钟/环境输入，任何 Python 3.9+ 重跑结果一致。**它是本记录的证据材料，不是可复用 Harness 组件，故不在 `manifest.json` 注册**（设计方裁定）。 |
| `evidence/boundary-cases.md` | 52 个定向用例逐条表：输入字节（转义呈现）、期望分支、实际分支、状态。 |
| `evidence/run-manifest.md` | 算法、输入域、固定 seed、迭代数、复现命令与结果摘要。 |
...
exit=0
stdout: result digest (sha256 of sorted result lines): 212e19d93f82674f3f757cb7f9472a1023eeb6ff310c9dc7aa2ae5bcc2c231a1
        （同流附记：directed cases: 52, failures: 0 / undefined/exception cases: 0）
stderr: <empty>
```

定向用例 52 个、失败 0；随机迭代 20 万、逃出划分 0；五个分支全部被覆盖，无死分支。

**方法学更正（本轮的核心教训）**：上一轮只做随机模糊测试并据此声称规则闭合，方法上是错的。随机扫描只能证明「没有输入落到划分之外」（全函数性），**不能证明「输入落进了正确的分支」**（判据正确性）。NEL / U+2028 / U+2029 这类反例即使被随机生成，也会因「有分支、无异常」而顺利通过——它们的问题从来不是无处可落，而是**落错了地方**。因此本轮的定向用例逐条携带期望分支，并从流程 §2.3 声明的对抗域**逐项**取材；B 组还额外包含集合外的边界邻居（U+00A0、U+2027、U+202F、U+2065、U+206A），因为只测集合内成员只能证明「不漏」，加测邻居才能证明「不误伤」。
