...
定向用例 69 个、失败 0（**分支与载体值双重比对**）；四类自检——比较器 5 项、共因 8 项、argv 24 项、认证 23 项——全部通过；**长度 0–2 全域穷举** 65,793 个输入、失配 0；长度 0–6 抽样 20 万次抽取（137,527 个唯一输入）、逃出划分 0、载体值失配 0——**抽样只对被抽到的输入成立，未被抽到的输入不在证明范围内**。
...
| **R3 [Important，A-I1 = B-I2]** ∀ 审计枚举基不是同一状态 | 接受。上一版记录的 175 是**跨状态混合值**（B 证明：该 HEAD 191、前一状态 157，记录的分布两者都不匹配）。现改为**提交前最后一步**在最终内容上重跑收集器，并迭代到不动点：命中 **267 行**，分布逐文件记录；新增第 18–24 行覆盖本轮出现的承载能力声明；**删除**「其余命中不构成全称能力声明」这句概括兜底——它本身就是又一个未经逐条检验的全称句；表基说明改为「**基于最终提交态的收集器命中集 + 人工分类**」。 |
