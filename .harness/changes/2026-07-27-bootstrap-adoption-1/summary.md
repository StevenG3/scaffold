...
定向用例 69 个、失败 0（**分支与载体值双重比对**）；比较器自检 5 项、共因自检 8 项、argv 自检 24 项均通过；**长度 0–2 全域穷举** 65,793 个输入、失配 0；长度 0–6 抽样 20 万次（137,527 个唯一输入）、逃出划分 0、载体值失配 0——**抽样只对被抽到的输入成立，未被抽到的输入不在证明范围内**。
结果摘要**绑定输入字节与实际载体值**。
