...
| Minor：缺红基线 | 已修：新增 `--baseline`（整改前判据），实测 **当轮历史值 69** 个用例中 **30 个见红**，同一批在当前判据下全绿（当前定向用例为 74，红基线仍为 30，见「当前结果」一节）。 |
...
| **F-R11-2** 再往前一行的廉价闭合 | 接受。守卫新增**层详情数字扫描**：CERTIFIED 在场时任何非零 `N mismatches` / `N failures` / `N undefined` / `N unrecognised` 即拒绝。验证者的两个外科式伪造（穷举 7、抽样 11 挂在 CERTIFIED 下）实测均被拒，并已作为两项自检回归固化。残余披露相应再次改写：按层表的最后一行由「未捕获」改为**捕获**；**当前真正剩下的只有一种**——把印出来的数字一并改掉的**整体伪造**，即渲染文本从头到尾自洽地说谎，与「oracle 本身说谎」同类，只能靠独立复现发现。 |
...
| **A-M1 / A-M2 / B-S-B3**（Minor） | 审计表第 23 行的「24 项」订正为 27（**审计表自身也漂移了**）；新增三行覆盖此前未成行的能力声明（定制记录的覆盖范围 → 已限定；自检不读共享环境状态 → 构造保证 + 回归指针；输入/载体从不截断 → 构造保证）；`summary` 的「20 万次抽取」统一为 `200,000 次抽取`，陈旧的 `5/8/24/12` 加当轮历史标注并指向当前值。 |
...
| **I2** 抽样句第三次复发 | 接受。`summary` 中的方法学段落改为**按时点限定**：「**早期只检查分支归属、尚无独立 oracle 的旧扫描器**只能证明……那是**当时**的局限；**当前**抽样层对每个命中输入核对分支与载体，不能证明的是**未抽中输入**」。全仓库记录再扫一遍，剩余同族措辞均已带历史限定。 |
...
| Finding | 裁决与整改 |
| --- | --- |
| **1** `protocol-runs.md` 当前条目沿用了上一提交的数值 | 接受，而且这是**本轮最难看的一条**：这份文件正是为「未入库的证据等于不存在」而建，它的当前条目却**逐字沿用 `bfbd546` 的数值**（脚本 SHA `3d3c46bf`、摘要 `ada0846f`、定向 72）——**为防止陈旧转录而建的文件，自己成了陈旧转录**。整改：在**最终内容态**重跑机械步骤，粘贴**本提交**的实测值（脚本 SHA、摘要、定向数、前驱 SHA、增量统计）。 |
...
| **Minor** 历史标注不统一 | 已统一：`run-manifest` 的 M10 段、`summary` 的红基线 Minor 行、`carrier_sweep.py` 的 oracle 注释三处裸 69 均加上当轮历史标注并指向当前值 74。 |
...
| **I2** 自指统计第三次复发 | 接受，并按设计方裁定做**类修复：去镜像**。记录里写「4 文件 `+42/-8`」而 Git 是「5 文件 `+60/-27`」——根因不是粗心，而是**把可事后导出的统计量抄进记录**：记录与它所在的提交互相追逐，永远差一步。整改：**删除记录中一切 Git/命令可导出的统计量**（差异统计、文件数、`rev-list` 计数），代之以**待执行的命令**；当前条目只保留两类事实——**机器生成产物本身**，以及**产物不变量**（脚本 SHA、结果摘要；它们描述的是产物内容，不随「哪个提交包含它们」而变，故无自指）。其余降为**带时点的历史**。PR 正文的 CI 指针改为 **exact-head** run，并把「push 后刷新 run id」写进指针刷新步骤。 |
...
| Finding | 裁决与整改 |
| --- | --- |
| **1（A-C1 = B-S-I2）终极类修复：把去镜像推到脚本可导出的一类** | 接受。第 11 轮我只删掉了 **Git 可导出**的统计量，规则本身写的却是「Git **或命令**可导出」——于是脚本可导出的一类（定向数、四类自检项数、穷举/抽样总数、唯一抽样数、分支直方图、各探针 digest）继续手抄，本次提交把认证自检从 27 改到 33，抄本没跟上，**第四次复发**。整改：**这些数字全部从当前语态记录中删除**，代之以「运行命令 / 读机器生成的 `boundary-cases.md`」；只保留两个**产物不变量**——脚本 SHA 与结果摘要（它们描述产物内容，不描述「哪个提交/哪次运行包含它」，故无自指）。历史小节保留其带时点标注的数字。**此后当前态数字只存在于机器生成产物中，陈旧数值这一类在结构上不再可能**——`protocol-runs.md` 中那句原本超前的断言，因此成为真陈述（已改写为该精确形态）。 |
...
| **3（A-I1 = B-M2）核对清单自相矛盾** | 接受。上一轮 I2 那一行期望「0」，而实际必然是「≥1」（病灶引述句本身含该模式），我却把它记成通过。现改为**锚定排除 + 真实期望**：命令用 `grep -v '^[0-9]*:|'` 排除清单自身那一行，期望恰 **2** 行并**逐行点名**（第 11 轮病灶描述、当前条目引述）。**核对命令若把自己算进去，就是又一个自指**——这句话已写进该行。全部 `<evidence>` 占位符替换为可直接运行的真实路径。 |
...
| **I1（Important）残值不止一处 + 无界声称** | 接受。会话 C 列出的九处（含逗号格式 `65,793` / `200,000` / `137,527` 导致初扫漏检——**这正是 ∀ 收集器不完备模式在其后继规则上原样复发**）已逐一处置：命令可推导的删值留命令、历史轮记录移入历史语境并绑定 HEAD、`boundary-cases.md` 可读的改为指针。另外两句无界声称——`run-manifest.md:430-431`「结构上不再可能」与 `protocol-runs.md:66`「当前态数字只存在于机器生成产物中」——**均被 C1 证伪**，已删除并改写为**有界**形态。 |
| **I2（Important）规则没有能失败的检查** | 接受，这是本轮最实质的一条。∀ 收集器匹配**关键词**而不匹配**数字**（`run-manifest.md:49` 根本不在其命中集内）；唯一相关的 checklist 行锚定在单一小节的两个字面短语上，残值一旦落在窗口外就恒返回 0。**「检查必须先证明自己能失败」这条纪律，在去镜像规则上缺位。** 整改：新增 `evidence/check_current_numbers.py`——扫描指定当前态区间内的全部数字 token（含逗号格式），减去**显式白名单**，剩余非空即 exit 1；并内置 `--self-test` 做**变异验证**（内存中注入一个杂散数字，断言必被抓住，不修改任何文件）。三条 checklist 行记录其正常态、变异验证与参数域。 |
