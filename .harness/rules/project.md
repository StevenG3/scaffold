...
门禁证据须记入 Change Record 的 `summary.md`。每条门禁一律记录**命令原文**与**退出码**；命令输出部分按下列三条穷举规则，覆盖门禁输出的全部情形：

- **(a) 命令有终态摘要行**（如 `unittest` 的 `Ran` / `OK` 行、`validate` 的结论行）：记录该行**原文**与退出码；不要求全量 verbose 输出。
- **(b) 命令成功且零输出**（stdout 与 stderr 均为空，如 `git diff --check`）：逐字记录 `<empty>` 与退出码。`<empty>` 是对「无输出」这一事实的结构化记法，不是伪造的命令输出。
- **(c) 禁止人工断言**：任何情形下都不得以「已检查」「均通过」之类的人工结论代替命令输出或上述记法。

规则 (a) 与 (b) 互斥且穷举：命令要么产出可摘要的输出，要么成功而无输出。若将来出现两者都不适用的门禁（例如成功却只输出无法摘要的二进制），须先回本规则补条款，不得临场自创记法。
