...
- **(b) 非空且整串可按 UTF-8 严格解码**：
  - **存在非空行**：记录**最后一个非空行**的原文。这是「终态摘要行」的可判定定义，取代任何依赖人工判断「哪一行算摘要」的说法；不要求记录全量 verbose 输出。允许额外附记同一流中的其他原文行（例如 `unittest` 的 `Ran` 行），但最后一个非空行是必记的下限。
    - **例外（该行不可无损呈现时）**：仅当该行（去除末尾 `0x0D` 后）解码所得的码点**全部不属于**下列**不可呈现码点集**时，才以原文记录；只要含有其中任意一个码点，改为记录**该流的字节数与 SHA-256**。此类码点在 Markdown 记录中不可见、会造成视觉断行或改变显示方向，无法无损无歧义呈现；若强求「记录原文」，记录者只能临场自创转义记法——而那正是本规则禁止的。本例外与 (b) 无非空行分支、(c) 分支采用同一载体形式。
      ...
      **不可呈现码点集**（固定枚举，逐条列出，不依赖任何 Unicode 版本的属性表，因此判定结果与 Unicode 版本无关）：
      ...
      | 双向控制符 | `U+200E`、`U+200F`、`U+202A`–`U+202E`、`U+2066`–`U+2069` |
      | BOM / ZWNBSP | `U+FEFF` |

      该集合是**穷举的白名单补集**：集合之外的一切码点（含 CJK、假名、谚文、emoji、组合记号等可见文本）均可原文记录。选择固定枚举而非 Unicode 类别（`Cc`/`Cf`/`Zl`/`Zp`）判定，是为了让判定不随 Python 或 Unicode 版本升级而漂移；代价是新版 Unicode 若引入新的不可见控制字符，须回本规则补条目。
