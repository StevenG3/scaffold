...
- **(b) 非空且整串可按 UTF-8 严格解码**：
  - **存在非空行**：记录**最后一个非空行**的原文。这是「终态摘要行」的可判定定义，取代任何依赖人工判断「哪一行算摘要」的说法；不要求记录全量 verbose 输出。允许额外附记同一流中的其他原文行（例如 `unittest` 的 `Ran` 行），但最后一个非空行是必记的下限。
    - **例外（该行不可无损呈现时）**：若该行去除末尾 `0x0D` 后含有 C0 控制字节（`0x00`–`0x1F`）或 `0x7F`，则改为记录**该行的字节数与 SHA-256**。此类字节无法在 Markdown 记录中无损、无歧义地呈现，若强求「记录原文」，记录者只能临场自创转义记法——而那正是本规则禁止的。本例外与 (b) 无非空行分支、(c) 分支采用同一载体形式。
