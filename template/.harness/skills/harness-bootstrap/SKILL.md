...
The record covers **every customized target asset** the bootstrap touched. A target asset is a Harness asset that exists to configure or govern the project. One row per asset, including all of:
...
**Explicitly out of scope: this Change Record's own audit metadata** - `summary.md`, `spec.md`, `tasks.md`, `customization-record.md`, and anything under the `originals/` subdirectory. Those files carry the evidence; they are not the objects being customized, so they never get a row.

No target asset is exempt; audit metadata is excluded by definition. A target asset that was touched but is hard to classify still gets a row; it is never omitted.
...
| `manifest.json` (`project-rules`) | `added` | Registers the new project rules file at `rules/project.md` so the validator sees it. | `project` |
...
> This project has no version control, so the pre-bootstrap originals of every `modified`, `replaced`, and `removed` target asset are stored under `originals/` in this Change Record directory.
