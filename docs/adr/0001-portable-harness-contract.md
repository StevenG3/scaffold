...
Manifest 中的路径使用 POSIX `/` 并相对自身解析；反斜杠和 Windows 盘符前缀非法，由校验器稳定报错，其余合法路径段再转换为当前系统的本地路径。组件通过 `id`、`kind`、`path` 描述，不提供强制的 Owner 字段，也不把示例 Coordinator 设为 Schema 保留角色。内置类型保持最小，扩展使用 `x-*` 命名空间。
