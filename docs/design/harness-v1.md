...
- 命令级错误（退出码 `2`，发生在参数解析成功之后）同样遵守 `--format`：`json` 下向 stdout 输出完整 envelope（`ok: false`、`command`、`errors`、`notices` 及该子命令的附加字段）后以 `2` 退出；`text` 下向 stderr 输出 `[CODE] path: message` 行。本条覆盖两类：可预期参数/环境错误（非法 `--adapters` 成员 `ARGUMENT_INVALID`、根目录不可读 `ROOT_UNREADABLE`），以及节点安全核验通过后仍发生的运行时 I/O 故障（读取失败、写入/替换失败）——后者归类退出 `2` 但不得逃逸到裸 `INTERNAL_ERROR` 文本。运行时 I/O 故障按发生的命令与阶段映射到稳定错误码：投影阶段 `PROJECTION_IO_ERROR`、`init` 生命周期（§7.2）`INIT_IO_ERROR`、统一入口 `validate`（§7.4）`VALIDATE_IO_ERROR`。节点布局类契约错误（§8.3）始终是退出 `1`，不得混入本类。命令级错误的 Text 渲染必须与 `emit` 相同地对 code/path/message 做控制字符转义（覆盖 C0、DEL、NEL、U+2028、U+2029），保证一个错误恰占一个物理行。参数解析本身失败（未知子命令、未知选项，此时 `--format` 不可知）仍走 stderr 文本，且同样经上述转义——解析器错误消息中由输入注入的控制字符不得拆行。
- **文件读取的运行时故障统一枚举三类异常**：`OSError`（含 `shutil.Error`）、`json.JSONDecodeError`，以及 UTF-8 解码失败 `UnicodeDecodeError`。后者是 `UnicodeError` 的子类，**既不是 `OSError` 也不是 `json.JSONDecodeError`**，因此凡以 `read_text(encoding="utf-8")` 读取契约文件（Manifest 读取、`validate_harness` 内部读取、投影目标读取）的边界都必须显式捕获 `UnicodeError`，否则真实非法 UTF-8 字节会逃逸为裸 `INTERNAL_ERROR`。三类异常同属运行时 I/O 故障，按上一条的阶段规则映射为 `INIT_IO_ERROR` / `PROJECTION_IO_ERROR` / `VALIDATE_IO_ERROR`，退出 `2`。捕获范围仅限这三类明确异常，不得以裸 `Exception` 兜底再伪装成 I/O 错误。
- **JSON 输出必须始终是合法 UTF-8**。POSIX 下 argv 经 `surrogateescape` 可能携带无法编码为 UTF-8 的代理字符：任何 argv 值含此类字节时，在进入子命令逻辑前稳定拒绝——报 `ARGUMENT_INVALID`、退出 `2`、经转义的单行 stderr 文本（该场景不产生 JSON envelope：非法字节使 envelope 本身无法成为合法 UTF-8）。命令级错误 envelope 自身始终由 ASCII 错误码、经转义的 path/message 与结构化字段组成，保证输出为合法 UTF-8 JSON。
- **错误 envelope 必须如实报告进度，且遵循 `projected_files` 语义 A**：`adapt` / `init` 不是跨文件事务；失败发生时，envelope 的 `written`（`adapt`）/ `projected_files`（`init`）必须列出**本次命令实际写入或原子提交（`os.replace` 落定）到磁盘的**投影文件，不得报告空列表冒充失败。**语义 A**：预先存在且 byte-identical、本次并未写入的投影（`unchanged`）**不得**计入 `written` / `projected_files`——它们不是本次的进度。若成功响应需同时报告「当前有效但本次未写入」的投影，必须使用独立的明确字段（如 `adapt` 的 `unchanged`），绝不混入 `written` / `projected_files`。`validate.py` 独立命令保持其既有行为不变；`harness.py validate` 的正常校验输出仍与 `validate.py` 逐字节一致，仅命令级错误路径按本条统一渲染。
...
3. 完整复制 `.harness/` 到 `<target>/.harness/`（含 `bin/` 自身，使目标项目可独立运行后续命令）。复制排除的缓存产物为确定清单：`__pycache__`、`*.pyc`、`.pytest_cache`、`.mypy_cache`、`.ruff_cache`、`.DS_Store`。**init 全生命周期错误契约**：生命周期定义为**从参数解析成功开始，直到成功或失败响应完成为止**；期间**所有**运行时 I/O 失败（`shutil.Error` / `OSError` / 读取时的 `json.JSONDecodeError` / UTF-8 解码的 `UnicodeDecodeError`（`UnicodeError`）竞态）都不得逃逸为裸 `INTERNAL_ERROR`。至少枚举下列阶段：①源 Harness 校验；②源 Manifest 再读取与解码；③`target` 路径检查与**规范化绝对路径解析**；④`copytree`；⑤目标 Manifest 读取、`origin` stamp 与写回；⑥`adapt` 投影阶段；⑦最终 `validate`；⑧成功响应前目标绝对路径的取得；⑨失败响应所需 cleanup/progress 状态的构造。逐阶段分类：源校验、源 Manifest 读取、`target` 规范化解析、复制、`origin` 写回、最终 `validate` 的文件读取与解码：报 `INIT_IO_ERROR`、退出 `2`；投影阶段沿用 §8.3 / §7.1 的 `PROJECTION_*` 分类（节点布局退出 `1`、运行时 I/O `PROJECTION_IO_ERROR` 退出 `2`）。全部按 §7.1 规则渲染（json 下输出完整 init envelope）。**成功响应前不得存在未纳入错误边界的裸操作**：目标的规范化绝对路径应在 target 检查阶段（复制之前）安全解析并缓存，成功与失败 envelope 复用该缓存；若该解析在复制前失败，返回 `INIT_IO_ERROR`、退出 `2`，此时尚无 `.harness/` 落盘，通常无需 cleanup hint。只要失败发生时目标 `.harness/` 已存在于磁盘（无论完整或部分），envelope 必须附带 `INIT_CLEANUP_HINT` notice 指明清理路径、返回准确 `target`，并保留现场不做部分回滚；envelope 的 `projected_files` 按语义 A 报告**本次实际写入/原子提交**的投影——**该进度要求对退出 `1` 的契约错误（如投影节点布局错误、最终校验失败）同样适用**，不得固定为空列表，也不得混入 `unchanged`。
...
JSON 附加字段：`target`（目标 `.harness/` 的规范化绝对路径，在复制前安全解析并缓存）、`projected_files`（**语义 A**：本次命令实际写入或原子提交的投影文件相对目标项目根的路径列表；预先存在且 byte-identical、本次未写入的 `unchanged` 文件不计入）。
...
- **adapt 生命周期错误边界**：以下运行时故障均进入稳定的 adapt 错误 envelope（`PROJECTION_IO_ERROR`、退出 `2`、按 §7.1 渲染完整 adapt envelope，不得逃逸为裸 `INTERNAL_ERROR`）：`validate_harness(root)` 自身的运行时文件读取/解码故障（present-but-unreadable 文件的 `OSError`、非法 UTF-8 的 `UnicodeDecodeError`）；校验成功后的 Manifest 再读取；UTF-8 解码竞态；投影读取、临时写、`fsync`、`close`、`replace`。其中根本身缺失或不可访问仍维持 `ROOT_UNREADABLE`（退出 `2`）；节点布局问题继续使用 `PROJECTION_*` 契约错误与退出 `1`。`--check` 必须保持零写入。
- `written` 遵循 §7.1 语义 A：仅列出本次实际写入/原子提交的投影；预先存在且 byte-identical 的文件计入 `unchanged`，绝不混入 `written`。

JSON 附加字段：`written`（**语义 A**：本次实际写入/原子提交的文件列表）、`unchanged`（本次未改动的既有有效投影）、`stale`（仅 `--check` 下）。
...
- **正常校验结果**与 `validate.py` 完全一致（内部复用其实现）：对同一夹具，退出码、stdout、stderr 在 Text/JSON 下与独立 `validate.py` 逐字节一致，仅作为统一入口提供。
- **代理字符校验结果的裁决（设计方，回应第七轮复审）**：v0 渲染器对含 POSIX `surrogateescape` 代理字符的校验结果输出无法按 UTF-8 解码的字节流，判定为 **v0 潜在缺陷**——v0 设计 §9.2 承诺的「确定性 Text/JSON 输出」以可被标准消费者解析为前提，非法字节流不在受保护行为之内。据此**批准对 `validate.py` 渲染层的一次同步加固**：在共享转义边界（Text 的 `escape_text_field` 与 JSON 渲染的字符串出口）对 U+D800–U+DFFF 代理字符执行确定性 ASCII 转义（形式由实现确定，须确定性、信息可辨、输出为纯 ASCII 序列）。兼容性承诺相应重述为：**凡字符串不含代理字符的校验结果（即一切原本可编码的输出），Text/JSON 逐字节与既有 v0 输出一致**；含代理字符的结果此前本就是非法字节流，无兼容性可言。统一入口继续委托 `validate.py` 渲染器，两者对全部输入保持逐字节一致——单一安全 JSON 出口由构造保证，§7.1 与本节自此对全部输入同时成立。本裁决同时覆盖 `validate.py` 独立命令的 **stderr 诊断行**（`[ARGUMENT_INVALID]` / `[ROOT_UNREADABLE]` / `[INTERNAL_ERROR]`）：其插值的异常消息同样经共享转义边界（`escape_text_field`，含控制字符与代理字符）渲染——含非法字节或换行注入的诊断行此前同属不可消费输出，无兼容性可言；不含此类字符的诊断行逐字节不变。以上（渲染层 + stderr 诊断转义）合并为对「`validate.py` 独立命令契约不变」约束的**唯一一组**输出行为例外；参数、退出码、输出流向不变，既有 `tests/test_validate.py` 不修改。至此全部终端出口（两个 CLI 的 stdout 与 stderr、Text 与 JSON）均经统一转义边界，「合法 UTF-8 且一错一行」对任意输入成立。
- **命令级运行时错误的对称边界**：`cmd_validate` 调用 `validate_harness` 时，除 `RootUnreadableError`（→ `ROOT_UNREADABLE`）外，present-but-unreadable 文件抛出的 `OSError`、以及 `read_text(encoding="utf-8")` 抛出的 `UnicodeDecodeError`（`UnicodeError`）都必须捕获并渲染为统一 CLI 的命令级错误 envelope：稳定错误码 `VALIDATE_IO_ERROR`、退出 `2`、遵守 `--format`（`json` 下向 stdout 输出含 `ok:false`、`command:"validate"`、`errors`、`notices` 的完整合法 UTF-8 envelope，stderr 为空；`text` 下向 stderr 输出经控制字符转义的单行 `[CODE] path: message`）。不得逃逸为裸 `INTERNAL_ERROR`。这是 §7.1 命令级错误契约在 `validate` 入口的对称落实，`init`/`adapt` 的边界不能替代它。
- `validate.py` 作为独立命令继续存在且契约不变（包括其对上述运行时故障沿用 v0 的裸 `INTERNAL_ERROR`/退出 `2` 行为）；文档以 `harness.py validate` 为推荐入口。字节兼容要求仅约束**正常校验结果**，命令级运行时错误路径按本节按入口各自渲染。
...
- 参数解析成功之后发生的运行时 I/O 故障（读取/解码/写入/替换失败）走退出码 `2`，使用稳定错误码 `INIT_IO_ERROR`（`init` 生命周期，§7.2）、`PROJECTION_IO_ERROR`（投影阶段，§8.3）、`VALIDATE_IO_ERROR`（统一入口 `validate`，§7.4），按 §7.1 命令级错误规则渲染，绝不逃逸为裸 `INTERNAL_ERROR`。
