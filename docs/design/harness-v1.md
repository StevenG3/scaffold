...
- Text 错误格式沿用 `[CODE] path: message`；JSON 输出固定包含 `ok`（布尔）、`command`、`errors`（数组，元素含 `code`、`path`、`message`）与各子命令附加字段。
...
3. 完整复制 `.harness/` 到 `<target>/.harness/`（含 `bin/` 自身，使目标项目可独立运行后续命令）。复制排除 `__pycache__` 与隐藏缓存。
...
- 目标文件不存在：创建文件，内容为受管块（加一行文件级说明）。
- 目标文件存在但无标记：在文件**末尾追加**受管块，用户原文一字不动。
- 目标文件存在且有一对标记：仅替换标记之间的内容。
- 标记不成对（只有 BEGIN 或只有 END，或顺序颠倒）：报错退出 `1`（code `PROJECTION_MARKER_BROKEN`），绝不猜测修复。
